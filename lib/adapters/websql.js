...
function openDB() {
  if (typeof window !== 'undefined') {
    if (window.navigator && window.navigator.sqlitePlugin &&
        window.navigator.sqlitePlugin.openDatabase) {
      ...
      return window.sqlitePlugin.openDatabase
        .apply(window.sqlitePlugin, arguments);
    } else {
...
function idbError(callback) {
  return function (event) {
    utils.call(callback,
               errors.error(errors.WSQ_ERROR, event.target, event.type));
  };
}
...
function webSqlPouch(opts, callback) {
  ...
  if (!db) {
    console.log('OPENED', name, Object.keys(cachedDatabases).length);
