...
function LevelPouch(opts, callback) {
  ...
  if (STORES[opts.name]) {
    db = STORES[opts.name];
    stores[DOC_STORE] = db.sublevel(DOC_STORE, {valueEncoding: 'json'});
    stores[BY_SEQ_STORE] = db.sublevel(BY_SEQ_STORE, {valueEncoding: 'json'});
    stores[ATTACH_STORE] = db.sublevel(ATTACH_STORE, {valueEncoding: 'json'});
    stores[ATTACH_BINARY_STORE] = db.sublevel(ATTACH_BINARY_STORE, {valueEncoding: 'binary'});
    opened = true;
    process.nextTick(function () { call(callback, null, api); });
  } else {
    db = sublevel(levelup(opts.name, opts));
    STORES[opts.name] = db;
    stores[DOC_STORE] = db.sublevel(DOC_STORE, {valueEncoding: 'json'});
    stores[BY_SEQ_STORE] = db.sublevel(BY_SEQ_STORE, {valueEncoding: 'json'});
    stores[ATTACH_STORE] = db.sublevel(ATTACH_STORE, {valueEncoding: 'json'});
