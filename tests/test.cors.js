...
function enableCORSCredentials(dburl, callback) {
  ...
  ajax({url: host + '_config/cors/credentials', json: false,
    method: 'PUT', body: '"true"'}, function(err, resBody, req) {
      call(callback, err, req);
  });
}
...
function tearDownAdminAndMemberConfig(dburl, callback) {
  ...
  ajax({
    ...
    method: 'DELETE',
    json: false,
    auth: {username: 'TestAdmin', password: 'admin'}
...
asyncTest('Cookie Authentication with Admin.', function() {
  ...
  testDB.put({
    _id: '_security',
    'admins': {'names': ['TestAdmin'], 'roles': []},
    'members': {'names': ['TestUser'], 'roles': []}
  }, function(err, res) {
    //add an admin and user
    setupAdminAndMemberConfig(name, function(err, info) {
      //--Run tests (NOTE: because of how this is run, COR's credentials must
      // be sent so that the server recieves the auth cookie)
      var instantDB = new Pouch(name, {cookieAuth: {
        username: 'TestAdmin',
        password: 'admin'
      }, withCredentials: true}, function(err, info) {
        ok(err === null, 'Cookie authentication.');
      });
      instantDB.post({
        _id: '_design/testdesign',
        views: {test_view: {map: 'function(doc){emit(doc._id,doc._rev);}'}}
      }, function(err, info) {
        //add design doc (only admins can do this)
